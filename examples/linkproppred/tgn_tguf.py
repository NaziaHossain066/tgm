...
from tgm.constants import METRIC_TGB_LINKPROPPRED, PADDED_NODE_ID
from tgm.core._storage.backends.tguf_backend import DGStorageTGUFBackend
from tgm.data import DGDataLoader, TGUFData
from tgm.hooks import (
    DeduplicationHook,
    HookManager,
    RandomNegativeEdgeSamplerHook,
    RecencyNeighborHook,
    TGUFPrecomputedNegativeEdgeSamplerHook,
)
...
parser = argparse.ArgumentParser(
    description='TGN LinkPropPred Example (tguf-backed)',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument('--seed', type=int, default=1337, help='random seed to use')
parser.add_argument('--dataset', type=str, default='tgbl-wiki', help='Dataset name')
parser.add_argument('--tguf-path', type=str, required=True, help='Path to .tguf file')
...
def train(
    ...
    for batch in tqdm(loader, file=sys.stdout, disable=None):
        opt.zero_grad()
...
full_data = TGUFData.from_tguf(args.tguf_path, time_delta='s')
ts, vs, tst = (
    full_data.store.train_split,
    full_data.store.val_split,
    full_data.store.test_split,
)
train_data = full_data.slice(ts.start, ts.end)
val_data = full_data.slice(vs.start, vs.end)
test_data = full_data.slice(tst.start, tst.end)

train_dg = DGraph(train_data, device=args.device, storage_backend=DGStorageTGUFBackend)
val_dg = DGraph(val_data, device=args.device, storage_backend=DGStorageTGUFBackend)
test_dg = DGraph(test_data, device=args.device, storage_backend=DGStorageTGUFBackend)
...
hm.register(
    'train', RandomNegativeEdgeSamplerHook(low=int(dst.min()), high=int(dst.max()))
)
hm.register('val', TGUFPrecomputedNegativeEdgeSamplerHook())
hm.register('test', TGUFPrecomputedNegativeEdgeSamplerHook())
